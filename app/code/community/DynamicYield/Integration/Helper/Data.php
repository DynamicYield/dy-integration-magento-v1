...
class DynamicYield_Integration_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const CONF_CATEGORY_TREE = 'dyi_config/product_feed/website_category_root';
    ...
    public function getExportableAttributes()
    {
        ...
        if (sizeof($attributeIds) > 0) {
            ...
            if (($key = array_search(static::BASE_PRICE, $attributeIds)) !== false) {
                unset($attributeIds[$key]);
            }
    ...
    {
        return Mage::getStoreConfig(static::CONF_CATEGORY_TREE) != '' ? explode(',',Mage::getStoreConfig(static::CONF_CATEGORY_TREE)) : null;
    }
    ...
    /**
     * Check if a product exists with given sku. This is used as a workaround to filter out custom options and 3rd party implementations.
     *
     * @param $product
     * @return bool
     */
    ...
    {
        return (bool)Mage::getModel('catalog/product')->getIdBySku($product->getSku());
    }
